...
export function OAuth(options: OAuthOptions): AuthClient {
  ...
  async function authorize(redirectURI?: string): Promise<{
    ...
    if (!redirectURI) {
      throw new Error(
        "Redirect URI should be specified either in the constructor or passed as an argument"
      );
    }
  ...
  async function callback(
    ...
    redirectURI?: string
  ): Promise<oauth.OAuth2TokenEndpointResponse> {
    if (!redirectURI) {
      redirectURI = options.redirectURI;
    }
    ...
    if (oauth.isOAuth2Error(params)) {
      logger.error(params, "oauth response error");
      throw new AuthError();
    }
    ...
    if (oauth.isOAuth2Error(result)) {
      logger.error(result, "OAuth response error");
      throw new AuthError();
    }
